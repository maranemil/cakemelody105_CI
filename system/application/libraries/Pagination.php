...
class MY_Pagination
{
    ...
    public function create_links()
    {
        ...
        $num_pages = (int)round(ceil($this->total_rows / $this->per_page));
        ...
        if ($this->page_query_string === true || $CI->config->item('enable_query_strings') === true) {
        ...
        $this->cur_page = round($this->uri_segment / $this->per_page) + 1;
        ...
        if ($this->cur_page < 0) {
            $this->cur_page = 0;
            //$this->cur_page = abs($this->cur_page);
        }
        ...
        // Is pagination being used over GET or POST?  If you get, add a per_page query
        // string. If posted, add a trailing slash to the base URL if needed
        if ($this->page_query_string === true || $CI->config->item('enable_query_strings') === true) {
        ...
        for ($loop = $start - 1; $loop <= $end; $loop++) {
            ...
            if ($i >= 0) {
                if ($this->cur_page === $loop) {
                    $n = round($this->uri_segment / $this->per_page) + 1;
                    //$output .= $this->cur_tag_open.$loop.$this->cur_tag_close; // Current page
                    $output .= $this->num_tag_open . '<a href="' . $this->base_url . $n . '" class="selected">' . $loop . '</a>' . $this->num_tag_close;
                } else {
                    $n = ($i === 0) ? '' : $i;
