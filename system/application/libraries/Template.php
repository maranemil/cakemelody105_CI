...
class CI_Template
{
    ...
    public function __construct()
    {
        ...
        // Load the template config file and set up our master template and regions
        include(APPPATH . 'config/template' . EXT);
        if (isset($this->template)) {
            $this->config = $this->template;
            $this->set_template($this->template['active_template']);
        }
    }
    ...
    public function initialize($props)
    {
        ...
        if (isset($props['template'])
            && (file_exists(APPPATH . 'views/' . $props['template']) || file_exists(APPPATH . 'views/' . $props['template'] . EXT))) {
    ...
    public function write_view($region, $view, $data = null, $overwrite = false)
    {
        ...
        if (count($args) > 1) {
            foreach ($args as $suggestion) {
                if (file_exists(APPPATH . 'views/' . $suggestion . EXT) || file_exists(APPPATH . 'views/' . $suggestion)) {
     ...
     * @access  public
     *
     * @param string   CSS files to link, import or embed
    ...
    public function add_css($style, $type = 'link', $media = false)
    {
        ...
        switch ($type) {
            ...
            case 'import':
                $css = '<style>@import url(' . $filepath . ');</style>';
                break;
    ...
    public function render($region = null, $buffer = false, $parse = false)
    {
        ...
        if ($region) // Display a specific regions contents
        {
            ...
            foreach ($this->regions as $name => $dregion) {
                $this->output[$name] = $this->_build_content($dregion);
            }

            if ($this->parse_template === true || $parse === true) {
    ...
    public function _build_content($region, $wrapper = null, $attributes = null)
    {
        ...
        if (!isset($region['content']) || !count($region['content'])) {
            return false;
        }
        ...
        if (isset($region['wrapper'])) {
            ...
            $output .= substr($region['wrapper'], 0, -1);
