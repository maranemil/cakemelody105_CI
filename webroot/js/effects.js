...
const Effect = {
    ...
    Transitions: {
        ...
        flicker: function (pos) {
            let pos1 = ((-Math.cos(pos * Math.PI) / 4) + 0.75) + Math.random() / 4;
            return pos1 > 1 ? 1 : pos;
        },
        wobble: function (pos) {
...
Effect.ScopedQueue = Class.create(Enumerable, {
    ...
    add: function (effect) {
        ...
        const position = Object.isString(effect.options.queue) ?
...
Effect.Tween = Class.create(Effect.Base, {
    initialize: function (object, from, to) {
        ...
        const args = $A(arguments), method = args.last(),
...
Effect.Pulsate = function (element) {
    ...
    const oldOpacity = element.getInlineOpacity();
    const transition = options.transition || Effect.Transitions.sinoidal;
    ...
    play: function () {
        return new Effect.Parallel(
            this.tracks.map(function (track) {
                const ids = track.get('ids'), effect = track.get('effect'), options = track.get('options');
