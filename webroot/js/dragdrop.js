...
const Draggable = Class.create({
    initialize: function (element) {
        const defaults = {
            ...
            endeffect: function (element) {
                const toOpacity = Object.isNumber(element._opacity) ? element._opacity : 1.0;
    ...
    initDrag: function (event) {
        ...
        if (Event.isLeftClick(event)) {
            ...
            const pointer = [Event.pointerX(event), Event.pointerY(event)];
            const pos = Position.cumulativeOffset(this.element);
    ...
    startDrag: function (event) {
        ...
        if (this.options.scroll) {
            if (this.options.scroll === window) {
                const where = this._getWindowScroll(this.options.scroll);
...
const Sortable = {
    ...
    create: function (element) {
        ...
        (options.elements || this.findElements(element, options) || []).each(function (e, i) {
            const handle = options.handles ? $(options.handles[i]) :
    ...
    onHover: function (element, dropon, overlap) {
        ...
        if (overlap > .33 && overlap < .66 && Sortable.options(dropon).tree) {
            ...
            const nextElement = dropon.nextSibling || null;
    ...
    mark: function (dropon, position) {
        ...
        const offsets = Position.cumulativeOffset(dropon);
    ...
    _tree: function (element, options, parent) {
        const children = Sortable.findElements(element, options) || [];

        for (let i = 0; i < children.length; ++i) {
            const match = children[i].id.match(options.format);
    ...
    tree: function (element) {
        element = $(element);
        const sortableOptions = this.options(element);
        const options = Object.extend({
    ...
    sequence: function (element) {
        element = $(element);
        const options = Object.extend(this.options(element), arguments[1] || {});
    ...
    serialize: function (element) {
        element = $(element);
        const options = Object.extend(Sortable.options(element), arguments[1] || {});
...
Element.findChildren = function (element, only, recursive, tagName) {
    ...
    $A(element.childNodes).each(function (e) {
        if (e.tagName && e.tagName.toUpperCase() === tagName &&
            ...
            elements.push(e);
        if (recursive) {
            const grandchildren = Element.findChildren(e, only, recursive, tagName);
