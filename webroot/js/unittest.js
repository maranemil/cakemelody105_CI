...
// noinspection JSUnusedGlobalSymbols,JSUnresolvedFunction,JSDuplicatedDeclaration,DuplicatedCode,JSDeprecatedSymbols
...
Test.Unit.Assertions.prototype = {
  ...
  assertNotEqual: function(expected, actual) {
    let message = arguments[2] || "assertNotEqual";
    try { (expected !== actual) ? this.pass() :
  ...
  assertNull: function(obj) {
    let message = arguments[1] || 'assertNull';
  ...
  assertInstanceOf: function(expected, actual) {
    let message = arguments[2] || 'assertInstanceOf';
    try { 
  ...
  assertRaise: function(exceptionName, method) {
    let message = arguments[2] || 'assertRaise';
    try { 
      method();
  ...
  assertElementsMatch: function() {
    const expressions = $A(arguments), elements = $A(expressions.shift());
    if (elements.length !== expressions.length) {
      this.fail('assertElementsMatch: size mismatch: ' + elements.length + ' elements, ' + expressions.length + ' expressions');
      return false;
    }
    elements.zip(expressions).all(function(pair, index) {
      let element = $(pair.first()), expression = pair.last();
  ...
  for(let specName in spec) {
    switch(specName){
      ...
      case "teardown":
        ...
        break;
      default:
        const testName = 'test' + specName.gsub(/\s+/, '-').camelize();
        let body = spec[specName].toString().split('\n').slice(1);
